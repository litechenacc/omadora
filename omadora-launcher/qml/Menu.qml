...
Item {
  ...
  // Lets synchronous command launchers toggle this shared menu safely.
  function isOpen() { return root.opened }
  ...
  property bool calculatorInput: false
  property bool webSearchMode: false
  property bool aiSearchMode: false
  property int requestSerial: 0

  function calculatorPreview(expression) {
    var source = String(expression || "").trim()
    if (!source) return ""
    if (!/^[0-9 .+\-*/%()]+$/.test(source)) return "Invalid expression"
    try { var result = eval(source); return isFinite(result) ? String(result) : "Invalid expression" }
  ...
  function openWebSearch(query) {
    ...
    Util.execDetached("omadora-open-url " + Util.shellQuote("https://www.google.com/search?q=" + encodeURIComponent(text)))
  }
  function mathExpression(expression) {
    var source = String(expression || "").trim()
    return source && /^[0-9 .+\-*/%()]+$/.test(source) && root.calculatorPreview(source) !== "Invalid expression"
  }
  ...
  property int headerHeight: Math.max(Style.space(root.calculatorInput && root.mode === "input" ? 54 : 34), Style.font.title + Style.spacing.controlPaddingY * 2)
  ...
  function rebuildDmenuDisplay() {
    ...
    var sourceQuery = root.filterText.trim()
    var query = sourceQuery.toLowerCase()
    // Math is a first-class launcher result: selecting it returns the original
    // expression for the launcher to evaluate/copy/notify just like Calculate.
    if (!root.webSearchMode && root.mathExpression(sourceQuery)) {
      displayModel.append({ itemId: "calculator", kind: "dmenu", icon: "", iconFont: "", appIcon: "", appId: "", label: "= " + root.calculatorPreview(sourceQuery), value: "CALCULATE:" + sourceQuery, target: "", detail: sourceQuery, path: "", childCount: 0, action: "", provider: "", score: -2, section: "" })
    }
    // g Tab deliberately replaces the app list: search is always the first
    // empty-icon action, followed by matching Helium bookmark candidates.
    if (root.webSearchMode) {
      displayModel.append({ itemId: "web-search", kind: "dmenu", icon: "", iconFont: "", appIcon: "", appId: "", label: sourceQuery ? "Search Google for “" + sourceQuery + "”" : "Search Google", value: "WEBSEARCH:" + sourceQuery, target: "", detail: "", path: "", childCount: 0, action: "", provider: "", score: -1, section: "" })
    } else if (root.aiSearchMode) {
      displayModel.append({ itemId: "chatgpt-search", kind: "dmenu", icon: "", iconFont: "", appIcon: "", appId: "", label: sourceQuery ? "Ask ChatGPT “" + sourceQuery + "”" : "Ask ChatGPT", value: "CHATGPT:" + sourceQuery, target: "", detail: "", path: "", childCount: 0, action: "", provider: "", score: -1, section: "" })
    }
    for (var i = 0; i < root.dmenuOptions.length; i++) {
      ...
      var option = root.dmenuOptions[i]
      var structured = option && typeof option === "object"
      var raw = structured ? String(option.label || "") : String(option || "")
      var parts = raw.split("\t")
      var icon = structured ? String(option.icon || "") : (parts.length > 1 ? parts.shift() : "")
      var label = structured ? raw : parts.join("\t")
      var value = structured ? String(option.value || label) : label
      var appIcon = structured ? String(option.appIcon || "") : ""
      var bookmark = structured && !!option.bookmark
      // Bookmarks are regular launcher candidates. Dedicated Google/ChatGPT
      // modes suppress apps and actions while retaining bookmark matches.
      if (root.aiSearchMode) continue
      if (root.webSearchMode && !bookmark) continue
      if (query && label.toLowerCase().indexOf(query) < 0) continue
      displayModel.append({
        itemId: "dmenu." + i,
        kind: appIcon ? "app" : "dmenu",
        icon: icon,
        iconFont: "",
        appIcon: appIcon,
        appId: "",
  ...
  function activateIndex(index, fromPointer) {
    ...
    if (root.dmenuActive) {
      ...
      root.applyDmenuSelection(displayModel.get(index).value || displayModel.get(index).label)
      return
    }
  ...
  function openDmenu(payload) {
    ...
    calculatorInput = !!payload.calculator
    webSearchMode = false
    aiSearchMode = false
    ...
    activeMenu = "root"
    navStack = []
    // A caller can prefilter a generic selection list (for example, the
    // System launcher route) while retaining normal keyboard navigation.
    filterText = String(payload.initialQuery || "")
    selectedIndex = 0
      ...
      Item {
        ...
        Keys.onPressed: function(event) {
          ...
          if (event.key === Qt.Key_Delete) {
            ...
            event.accepted = true
          } else if (event.key === Qt.Key_Tab && root.dmenuActive && root.mode === "select" && !root.webSearchMode && !root.aiSearchMode && root.filterText === "g") {
            root.webSearchMode = true; root.setFilter(""); event.accepted = true
          } else if (event.key === Qt.Key_Tab && root.dmenuActive && root.mode === "select" && !root.webSearchMode && !root.aiSearchMode && root.filterText === "a") {
            root.aiSearchMode = true; root.setFilter(""); event.accepted = true
            ...
            if (root.webSearchMode || root.aiSearchMode) {
      ...
      Column {
        ...
        Rectangle {
          ...
          Text {
            ...
            text: root.webSearchMode ? (root.filterText || "Search web…") : (root.aiSearchMode ? (root.filterText || "Ask ChatGPT…") : (root.filterText || (root.dmenuActive ? (root.dmenuPrompt + "…") : ((root.item(root.activeMenu) ? (root.item(root.activeMenu).title || root.item(root.activeMenu).label) : "Go") + "…"))))
          ...
          Text {
            ...
            visible: root.calculatorInput && root.mode === "input"
            text: root.filterText ? "= " + root.calculatorPreview(root.filterText) : "Type an expression"
            color: root.foreground; opacity: 0.62; font.family: root.fontFamily
        ...
        Item {
          ...
          Column {
            ...
            Text {
              text: root.webSearchMode ? (root.filterText ? "Searching the web for “" + root.filterText + "”" : "Type a web search") : (root.aiSearchMode ? (root.filterText ? "Asking ChatGPT “" + root.filterText + "”" : "Type a ChatGPT prompt") : (root.filterText ? "No matches for “" + root.filterText + "”" : "Nothing here yet"))
              color: root.foreground
              opacity: 0.7
              font.family: root.fontFamily
